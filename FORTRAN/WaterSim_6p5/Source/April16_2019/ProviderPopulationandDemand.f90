...
Module lms_ProviderPopandDemand
 ...
 contains
    ...
    !           real, parameter :: lvf_CF_Hartmann=-3.7 ! NOAA CLIMAS 200909_Hartmann.pdf
            ...
            real :: lvf_temp,lvf_temp_2,lvf_temp_3 !,lvf_tempGPCD,lvf_tempGPCD2 !,lvf_add
            !
            real :: lvf_correctionDemand
...
!            E%MySub='calcDemand'
            ...
            !
             IF(gvl_errorFlag)THEN
                ...
                ! ----------------
                !
                if(gvl_waterSim5YN)gvl_IwaniecYN=.false.
...
!                         call embeddedCall(T)
              ...
              do j = 1,gvi_Providers,1
                ...
                if(T%year <=T%startyear)then
                  ...
                  ! 08.30.18 WaterSim 6 to WaterSim 5 correction
                    lvf_correctionDemand=1.0 !
                    gvf_TotalDemand_acft(j)=0.
                   gvf_TotalDemand_acft(j)=gvf_demandNoAg(j) *lvf_correctionDemand
                ! DAS 05.12.16
                ! ---------------------------------
                    ...
                    if(gpi_lclu <= T%year)then
                      ...
                      ! utilize waterUseByLCLU
                        ...
                        do k = 1,13,1
                            !
                            if(k == 1)then
                              go_demand_AG_AF(j)= gvf_waterUseLCLU(k,j)
                            endif
                            !
...
200 format(I4,1x,I2,1x,I2,1x,F9.1,1x,I4)
                  !  05.25.12
                   gvf_WaterDemand_acft(j,1,1)=anint(gvf_TotalDemand_acft(j)*lvf_correctionDemand)                           ! Total demand
                ...
                !`
                    ...
                    !
                     ...
                     !
                      ...
                      ! 06.05.15 das
                      !
                      ! 02.01.19 right now, always true (as default)
