            ...
            if( T%year < gvi_baseYear)then
              ...
              call ProviderArea2012(lvf_Area)
                !
                  ...
                  !
                    totalRoofArea=lpf_roofAreaHomes+lpf_roofAreaMFamily+lpf_roofAreaPeriUrbanHomes
                  !
...
920             format(I4,2x,I2,2x,I2,2x,3(F12.3,1x))
                ...
                !
               ! 
!                write(105,610)T%year,i,lvf_rainHarvesting_resPI(i),lvf_periUrban,go_OutDemand_lclu_AF(1,i), &
!                    gvf_rainfallfactor,gv_PopGrowthRateAdjPct(i),gvi_efficiencyLCLUres,gv_indexyearSVT,gv_indexyearCO
!                write(106,710)T%year,i,lvf_rainHarvesting_res(i),lvf_singleFamily,go_OutDemand_lclu_AF(2,i), &
!                    gvf_rainfallfactor,gv_PopGrowthRateAdjPct(i),gvi_efficiencyLCLUres,gv_indexyearSVT,gv_indexyearCO
!                write(107,810)T%year,i,lvf_rainHarvesting_resMF(i),lvf_multiFamily,go_OutDemand_lclu_AF(3,i), &
!                    gvf_rainfallfactor,gv_PopGrowthRateAdjPct(i),gvi_efficiencyLCLUres,gv_indexyearSVT,gv_indexyearCO
!610             format(I4,1x,I2,1x,3(F13.2,1x),2(F8.2,2x),5x,3(I4,3x))
!710             format(I4,1x,I2,1x,3(F13.2,1x),2(F8.2,2x),5x,3(I4,3x))
!810             format(I4,1x,I2,1x,3(F13.2,1x),2(F8.2,2x),5x,3(I4,3x))
                !
