 ...
 contains
    !
        ...
        subroutine AgricultureDemand(T,i,effluent)
            ...
            real :: lvf_AgEfficiency,gif_AgEfficiency
            ! =====================================
            ! - Type Constructs -
            type(runTime)T
            ! ===================
!                !
                ...
                lvf_AgEfficiency=1.0
                 gif_AgEfficiency=go_AgEfficiency
                if( 0 < go_AgEfficiency)lvf_AgEfficiency=1. + (1-(gif_AgEfficiency*0.01))
                !

                    if(gvl_IwaniecYN)then
                      GW =go_groundwaterAgProvider_AF(i)
                      SW =go_surfAgProvider_AF(i)
                      effluent=go_effluentAgProvider_AF(i) 
                      EFF=effluent
                    else
                ...
                ! Currently set at 2010 - 01.31.19
                if(T%year < gpi_lclu)then
                  ...
                  ! Demands
                     lvf_temp=0           
                     lvf_temp=(GW + max(0,SW-CAPtransfers)+EFF)
                    if(0 < lvf_AgEfficiency)lvf_temp=(GW + max(0,SW-CAPtransfers)+EFF)*lvf_AgEfficiency
                    !
...
!                          !
                    ...
                    endif ! Base year loop
!                  ! =====================================================================
                endif ! gpi_lclu loop
                    !
                    ! THis code is re-setting these output variables (to the interface)
                    ! 03.13.2019
                    ! ======================
                    if(gvl_IwaniecYN)then
                     go_groundwaterAgProvider_AF(i)=go_groundwaterAgProvider_AF(i) ! GW
                     go_surfAgProvider_AF(i)=go_surfAgProvider_AF(i) ! SW
                     go_effluentAgProvider_AF(i)=go_effluentAgProvider_AF(i)  ! EFF
                    else
        ...
        subroutine AgricultureProduction(T,effluent)
            ...
            real :: lvf_temp_1,lvf_temp_2,lvf_temp_3
            real :: lvf_unPumpedGW,gi_AgEfficiency
            ! =======================================
            !
            ! - Type Constructs -
            type(runTime)T
            ! ===================
                ...
                !
                  ...
                  ! Now scalled from 20 to 100 ?
                    gi_AgEfficiency=go_AgEfficiency
                  if(0 < go_AgEfficiency)lvf_AgEfficiency=1. + (1-(gi_AgEfficiency*0.01))
                  !
...
30 format(I4,1x,4(F10.1,1x))
                    ...
                    !
                        ...
                        lvf_kilosCurrent=((lpf_kilosPerAF*(lvf_AgEfficiency))*lvf_agTotalWaterUse)

                         lvf_agProduction=0
                        lvf_agProduction=lvf_kilosCurrent
                        ! THis was changed on 03.07.2019 
                        ! Attempting to account for changes in efficiency, and thus groundwater saved
                        lvf_unPumpedGW= max(0,  (lvf_kilosCurrent*(1/lpf_kilosPerAF)) - (lvf_kilosStatic*(1/(lpf_kilosPerAF)))  )
                    !
...
1000 format(I4,1x,5(F9.0,1x))
        ...
        subroutine AgriculturalBudgets(T,i,effluent,GW,lvf_AgToVadose,lvf_AgEvap)
            ...
            real :: lvf_weightedDemand(gvi_maxProV),lvf_totalDemand
            real :: lvf_tempADD,tempcredits
            real :: lvf_AgCreditWeight(gvi_maxProV)
            !
            ...
            ! ===================
                ...
                if(T%startyear < T%year)then
                    ...
                    if(gvl_waterSim5YN)then
                     call creditChanges(T%year,i,lvf_AgCreditWeight)
                    else
                      lvf_AgCreditWeight(i)=gvf_AgCreditWeight(T%year,i)
                    endif
                    !
                   if(gvi_baseYear <= T%year)then
                      ...
                      if(gvl_IwaniecYN)then
                         ...
                         lvf_Demand(i)=gvf_waterUseLCLU(1,i) !gvf_agDemand_acft(i)
                            lvf_totalDemand=sum(lvf_Demand)
                           lvf_weightedDemand(i)=lvf_Demand(i)/ lvf_totalDemand
                                               !
                          go_effluentAgProvider_AF(i)=0
                         if(0 < lvf_Demand(i))then
        !                     ! First Level
                              if(effluent < lvf_Demand(i))then
                               lvf_diffDemand(i)=lvf_Demand(i)-effluent
                               go_effluentAgProvider_AF(i)=effluent
                              else
                               lvf_excessEffluent(i)=effluent-lvf_Demand(i)
                             ...
                             ! Second Level
                                !temp=gvf_AgCreditWeight(T%year,i)
                                 temp=lvf_AgCreditWeight(i)
                                !
                                SW= go_surfaceAgTotal_AF(T%year-1)*temp
                              ...
                              if(lvf_temp <  lvf_diffDemand(i))then
                                ...
                                ! Third Level
                                GW = go_pumpingAgTotal_AF(T%year-1)*  lvf_weightedDemand(i)
                                go_groundwaterAgProvider_AF(i)=0
                              lvf_temp=GW
                              if(lvf_temp < lvf_diffDemand(i))then
                                lvf_diffDemand(i)=lvf_diffDemand(i)-lvf_temp
                            ...
                            !
                            ! Weighted demand for groundwater did NOT satisfy all GW needs
                            ! Assume sufficient GW to override
                            if(0 < lvf_unmetDemand(i))then
                                go_groundwaterAgProvider_AF(i)=go_groundwaterAgProvider_AF(i) + lvf_unmetDemand(i)
                                lvf_excessGW(i)=0
                                lvf_unmetDemand(i)=0
                            endif
                            !
                        ...
                        !
                        ! Ag acerage / total Acerage Ag for the Valley
                        !temp=gvf_AgCreditWeight(T%year,i)
                        temp=lvf_AgCreditWeight(i)
                        !
                        ! NOTE: go_pumpAgTotalEfficiency_AF(T%year-1) is defined in AgriculturalProduction 
                        ! subroutine (happens after this call
                     ...
                     SW=SW + gvi_WaterToAgriculture_acft_a(i)
                     !
                     ! 03.12.2019 new code here
                     ! ===================================
                     go_groundwaterAgProvider_AF(i)=GW
                     go_surfAgProvider_AF(i)=SW
                     go_effluentAgProvider_AF(i)=effluent
                     ! ===================================
                      !
                    ...
                    ! Start Year
                     temp=lvf_AgCreditWeight(i)
                    GW=gv_AgWaterPumpingSRV(T%year,6)*temp
                    SW=gv_AgWaterPumpingSRV(T%year,5)*temp
                    !
                endif
                    !
                    lvf_tempADD=lvf_tempADD+GW
                    tempcredits=tempcredits+lvf_AgCreditWeight(i)
                    ! Send to the Kernel Interface
                    ! ===================================
                    ! 04.15.19 I commented these out- found in Global Subroutines line 662
...
1000 format(I4,1x,5(F9.0,1x))
        ...
        ! --------------------------------------------------------------------
        subroutine creditChanges(year,provider,lvf_AgCreditWeight)
            integer :: provider,k,i
            integer :: year
            real :: lvf_totAcreage,lvf_relArea(gvi_maxProV)
            real :: lvf_AgCreditWeight(gvi_maxProV),lvf_addAcerage
                    !
                   if(provider < 2)then
                    call sMaskAgriculture(gvl_mask)
                     ...
                     lvf_addAcerage=0
                        do k = 1,gvi_Providers,1
                          if(gvl_mask(k))then
                            lvf_addAcerage=lvf_addAcerage + gvf_acerageAg(year,k) 
                          endif
                        end do
                          !
                        do i = 1,gvi_Providers,1
                          lvf_relArea(i)=0
                            if(0 < lvf_addAcerage)then
                              if(gvl_mask(i))then
                                lvf_relArea(i)=gvf_acerageAg(year,i) / lvf_addAcerage
                                ! 11.20.16
                                ! 04.13.19 das
                                lvf_AgCreditWeight(i)=lvf_relArea(i)
                              endif
                            else
                            endif
                        end do
