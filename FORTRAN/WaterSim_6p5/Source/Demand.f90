    ...
    subroutine sRainfallHarvesting(T,i,rain,lvf_rainWaterHarvest_mm,lvf_rainHarvesting)
      use lms_CitiWaterBudgets
       use gm_ModelControl
        ...
        real,parameter :: lpf_runoffCoeffMultiFamily=0.97
        real,parameter :: lpf_roofAreaHomes=232 ! m2 or 2500 ft2 (two pitch roof [plus garage] 60*25 feet [conservative]) 1400 ft2 house
       ! real,parameter :: lpf_peoplePerHouse=2.69 ! http://quickfacts.census.gov/qfd/states/04000.html
        real,parameter :: lpf_storageCapacity=1000 ! 1000 gallon storage tank $ 521.00 from plastic-mart

        real,parameter :: lpf_roofAreaMFamily=6412 ! 7544 reduce by 15% ! 92 m by 82 m active roof area
        real,parameter :: lpf_roofAreaPeriUrbanHomes=464 ! Double single family roof area
        ! Only 50% of multi-family buildings adopt this policy (above and beyond the general adoption curve)
        ! i.e., 25 % overall (50% of 50%)
        !real,parameter :: lpf_MFamilyAdoption=0.5
        !
            ...
            if( T%year < gvi_baseYear)then
              ...
              call ProviderArea2012(lvf_Area)
                !
                  ...
                  !
                  !lvf_houses=(1/lpf_peoplePerHouse * (go_PopulationOnProject(i)+  go_PopulationOther(i))) * lpf_singleFamily
                  ! 11.01.16
                  ...
                  * lvf_medDensityHousing * lvf_rainPotential_AF 
                  !
                  ...
                  * lvf_highDensityHousing * lvf_rainPotMulti_Fam_AF 
                  !
                  lvf_rainHarvesting_resPI(i)= lpf_runoffCoefficient *  lvf_actualHouses  & 
                  * lvf_lowDensityHousing * lvf_rainPotentialPeriUrban_AF  
                ...
                if(0 < lvf_singleFamily)then
                   lvf_gallonsAvailable= (lvf_rainHarvesting_res(i)* gpd_galperacft)/lvf_singleFamily/7
                  if(lpf_storageCapacity < lvf_gallonsAvailable)then
                   lvf_rainHarvesting_res(i)=(lpf_storageCapacity/ gpd_galperacft) * lvf_singleFamily *7
                  endif
                endif

                ! Peri-Urban - 3000 gallon tanks total
                if(0 < lvf_periUrban)then
                   lvf_gallonsAvailable= (lvf_rainHarvesting_resPI(i)* gpd_galperacft)/lvf_periUrban/7
                  if(lpf_storageCapacity*3 < lvf_gallonsAvailable)then
                   lvf_rainHarvesting_resPI(i)=((3*lpf_storageCapacity)/ gpd_galperacft) * lvf_periUrban *7
                  endif
                endif
                  !

               ! lvf_multiFamily - 7000 gallon tanks total
                if(0 < lvf_multiFamily)then
                   lvf_gallonsAvailable= (lvf_rainHarvesting_resMF(i)* gpd_galperacft)/lvf_multiFamily/7
                  if((lpf_storageCapacity*7) < lvf_gallonsAvailable)then
                   lvf_rainHarvesting_resMF(i)=((7*lpf_storageCapacity)/ gpd_galperacft) * lvf_multiFamily *7
                  endif
                endif
                !
              ...
              if(gvl_rainWaterHarvestResOnly)then
                ...
                if(gvl_rainWaterHarvestComOnly)then
                  ...
                  lvf_businesses=0.35 ! 35% of the business property is roof - Needs checking
                  ! 
