              ...
              if(lvl_firstLoop)then             
                ...
                if(lvl_firstLoop)then
                    ...
                    if(T%year < 2001 .and. i == 1)then
                      ...
                      ! Used in Agriculture.f90 - line 339  NOTE- test this code
                      if(gvl_IwaniecYN)then
                        gvf_pumpingAg=sum(go_groundwaterAgProvider_AF)
                      else
                ...
                !
!write(103,*)T%year,gof_regionalGW, lvf_regionalInputs,lvf_regionalOutflows
300 format (I4,1x,3(F15.2,1x)) 
...
!                    minPCD=water * gpd_galperacft * 1./ ( days * lpf_gallonsPerMinute *lvWB%lv_providerPop(i) )
        ...
        subroutine sEffluent(T,lvWB,i,vTstep)
            ...
            ! =====================
             ...
             !
                ...
                !
                ! Default settings until instructed to change to user inputs
                P%In_1=gvf_parm_EffluentToVadose(i)
                !P%In_1=gdf_parm_EFFtoVadose(i)

                P%In_2= gvf_parm_EffluentToPP(i)
                !P%In_2=gdf_parm_EFFtoPP(i)
                P%In_3=0
                P%In_4=0
                !
              ...
              ! These parameters will need checking in the interface
               ! to ensure that the sum <=1
                ...
                !
                    ...
                    ! Almost Zero Waste, Paleo Verde shuts down in 2040
                     lvf_propIwaniec=P%Out_2 * T%propYears_2040
                        !
                        ...
                        if(gvl_IwaniecScenarios_PPtoCities)then
                            ...
                            if(gvl_mask(i))then
                              ...
                              if(T%year < gvi_baseYear-1)then
                                ...
                                if(0 < gvf_parm_EffluentToVadose(i))then
                                  ! 02.25.19 new code for the six scenarios
                                    lvf_years=5
                                  call sRampParameters(T%policyYear,lvf_years,gdf_parm_EFFtoVadose(i),gvf_parm_EffluentToVadose(i),response)
                                endif
                                lv_fluxToVadose(i,vTstep)=response * lv_fluxFromWWTP(i,vTstep)
                              endif
                              !
                            endif
                        endif
                        !
                        gvi_retainVadoseFlux(i)=lv_fluxToVadose(i,vTstep)
                        !
                         lv_fluxToAgIrrigation(i,vTstep)=0
                        if(gvl_mask(i))then
                          lv_fluxToAgIrrigation(i,vTstep)=lv_fluxFromWWTP(i,vTstep)&
                ...
                ! CHECK THIS CODE 11.21.16 - I did. 02.21.19
                ...
                if(temp < lv_fluxFromWWTP(i,vTstep))then
                 ...
                 ! two cases in a default testing run. Rounding differences. For now, ignore.
                  if(lv_fluxFromWWTP(i,vTstep) < temp)then
                  else
                  endif
                 !
                    lvWB%lvf_fluxEffluentToPowerPlant(i,vTstep)= nint(lv_fluxToPowerPlant(i,vTstep))
        ...
        subroutine sReclaimed(T,lvWB,i,vTstep)
            ...
            ! =======================
              ...
              !
                lvf_tempSum(i)=lvWB%lvf_fluxReclaimedToOutput(i,vTstep)
              ! ---------------------
              ! Update states
              ! -----------------
              ! 
        ...
        subroutine sWWTP(T,lvWB,i,vTstep)
            ...
            ! ======================
                ...
                if(gvl_waterSim5YN)then
                    ...
                    ! ==========================
                    ! Almost Zero Waste
                    ! This global variable is used in the sRampUpParameters subroutine
                    if(gvi_IwaniecScenario == 6)gvl_IwaniecScenarios_PPtoCities=.true.
                     ! New code 02.22.2019
                    if(0 < gvf_parm_WWtoEffluent(i))then
                        ! Input from the interface, so override default
                      call sRampUpParameters(T%policyYear,gdf_parm_WWtoEFF(i),gvf_parm_WWtoEffluent(i),P%Out_1)
                        if(0 < gvf_parm_WWtoIndReuse(i)) P%Out_2=MAX(0,1-P%Out_1)
                    else
                      call sRampUpParameters(T%policyYear,gdf_parm_WWtoEFF(i),P%Out_1,P%Out_1)
                    endif
                    ! End New Code 02.22.2019
                !
                endif
                !
...
10 format(I4,1xI2,1x,3(F10.3,1x))
        ...
        subroutine sRampUpParameters(PolicyYear,InGlobal,InLocal,Out)
            ...
            ! ===============================
                !
                if(gvl_IwaniecScenarios_PPtoAg)then
                  lvf_localYears=gvf_YearsToInflection
                    if(gvl_IwaniecScenarios_PPtoCities)lvf_localYears=gvf_YearsToInflection/2
        ...
        subroutine sBalanceWS(T,lvWB,i,vTstep,lvWS)
           ...
           ! =====================================
                ...
                if(2000 < T%year)then
                 ...
                 lvf_ROreclaimed=gvf_ROreclaimedInput(T%year,i,1)
                endif
                !
                 lvf_BWaterNotUsed=0
        ...
        ! -------------------------
          ...
          subroutine sGrayWater(T,lvWB,i,vTstep,lvf_indoorWater,lvf_tWater,lvf_BW,lv_flux,lv_providerGPCD, &
            ...
            ! =======================
                  ...
                  if(lvWB%lvi_ResComInd == 1)then
                    ...
                    if(gvl_IwaniecYN)then
                     ...
                     if(0 < gvf_RainGrayCompliance)then
                      gvf_grayWaterCompliance=gvf_RainGrayCompliance*0.01             
                      gvf_yearsToAdoptGrayWater=gvf_yearsToAdoptRainWater
                    endif
                    !
                ...
                ! added this block on 08.27.18
                ! =======================================
                !if(gvf_RainGrayCompliance .eq. 0)then
...
!            real, parameter :: lpf_peoplePerHouse=2.59
        ...
        subroutine sRunoff(RCN,lvf_rainfall,fRunoff,lvf_Storage)
            ...
            ! =====================
                !
                ! rainfall in mm
                ! Curve number runoff equation - chapter 10 [Mockus (1949)]
        ...
        subroutine sRainfallRunoff(T,lvWB,i,vTstep)
            ...
            !
            real :: lvf_passRunoff_AF !,lvf_outRainHarvested
...
!                   call ProviderRCN(T,lvWB,i,lvf_xbarRCN) ! Area-weighted (land cover type) Runoff Curve Number
                    ...
                    !
                    ! No evaporation subtracted, but after rainwater harvesting (if any)
                    ! 12.10.18
                    go_GrossStormWater_AF(i)=lvf_newRunoff(i,vTstep)
                    ! lvf_outRainHarvested=0
                    !lvf_outRainHarvested=go_harvestRainWater_AF(i)
...
100          format(I4,2x,2(F12.4,1x))
        ...
        subroutine TypeTransferIN(T,i,vTstep,vPV,lvWB)
            ...
            ! =====================
                ...
                ! 04.08.16 das
                ! ======================
                ! Reduce leaks from current day value to approach zero by 2060
                 lvf_modLeaks=0
...
1003            format(I4,2x,I4,2x,I4,2x,42(F10.2,1x))
    ...
    !                 go_demand_AG_AF(i)=0
                    ...
                    ! START 07.07.17 DAS
                    go_demand_MD_AF(i)=gvf_waterUseLCLU(8,i)
                    go_demand_HD_AF(i)=gvf_waterUseLCLU(2,i)
                    go_demand_LD_AF(i)=gvf_waterUseLCLU(13,i)
                    go_demand_TurfTree_AF(1,i)=gvf_waterUseLCLU(4,i)
                    go_demand_TurfTree_AF(2,i)=gvf_waterUseLCLU(5,i)
                    go_demand_TurfTree_AF(3,i)=gvf_waterUseLCLU(10,i)
                    ! END 07.07.17 DAS edits
                    ...
                    if(gvl_APIcleared)then
                        ...
                        if(0 < lvWB%lvf_outdoorPotable(i) .or. 0 < lvWB%lvf_outdoorNonPotable(i))then
                            ! 06.04.18 this line looks like it could be a problem. not examining stormwater
