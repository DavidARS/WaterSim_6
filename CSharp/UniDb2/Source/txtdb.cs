...
//========================================================================================
// There is a bug in the DataAdapter code that does not work if there is a sp[ace in the table name
// 2 /28/17
...
namespace txtdb
{
    public class TxtDbDataAdapter : DbDataAdapter
    {
        ...
        private void SetupAdapter(string aCommand, TxtDbConnection theConnection, out bool IsErr, out string ErrMessage)
        {
            ...
            if (ParseSqlText(aCommand, out FSQLCommand, out FForString, out FFromString, out FWhereString))
            {
                FErrMEssage = "";
                FIsErr = false;
            ...
            else
            {
                if (TheColumns.Count == 0)
                {
                   //  Not sure how this can happen  for now throw and exception
                    throw new Exception("Odd error, now columns could be defined from the text data file");
                }
...
//            throw new Exception("Fill(DataTable dataTable) not implemented");
    ...
    //
        ...
        private bool ParseSqlText(string sqlText, out string CommandStr, out String FromStr, out string SourceStr, out string WhereStr)
        {
            bool result = false;
            FromStr = "";
            SourceStr = "";
            ...
            if ((index < 0) || (str1.ToUpper() != SelectCommandString))
            {
    ...
    #region TxtDbCommandRegion
    {
        ...
        public const string CreateTableCommandString = "CREATE TABLE";
        public const string CreateDatabaseCommandString = "CREATE DATABASE";
        public const string CreateViewCommandString = "CREATE VIEW";
       ...
       //Summary:
        ...
        public override int ExecuteNonQuery()
        {
            if (FCommandText != "")
            {
                string CommandStr = FCommandText.ToUpper();
                switch (CommandStr)
                {
