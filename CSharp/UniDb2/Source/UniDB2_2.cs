 ...
 * 
 *   Version 1 Copyright Ray Quay 2013
 *   
 *   Unified SQL Support 
 *   
                ...
                case SQLServer.stAccess:
                    {
                        ...
                        FOleDbConnection = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;; data source=" + DatabaseName + ";User Id=" + UserName + ";Password=" + PassWord + ";" + OtherConnectStringOptions);
 ...
 //       public DbCommand CreateCommand()
    ...
    public class UniDataAdapter : System.Data.Common.DbDataAdapter
    {
       ...
       // public void Fill(DataTable DT)
        ...
        ///-------------------------------------------------------------------------------------------------
        /// <summary>   Gets the data adpater. </summary>
        ///
        /// <value> The data adpater. </value>
        ///-------------------------------------------------------------------------------------------------

        public System.Data.Common.DataAdapter DataAdpater
        {
            get { return FDataAdapter; }
        }
    }
    // /////////////////////////////////////////////////////////////////////////////////////////////////////
    // 
    ...
    public static class Tools
    {
        ...
        ///-------------------------------------------------------------------------------------------------
        /// <summary>   Creates an update all data adapter. This creates UniDbAdapter that will update a
        ///             database with the spcified table, updating all records and all fields. </summary>
        ///
        /// <exception cref="Exception">    Thrown when SQLServer type ois txtdb. </exception>
        /// <cref = "UniDataAdapter">
        ///
        /// <param name="TheTable">         the table. </param>
        /// <param name="Tablename">        The tablename. </param>
        /// <param name="TheDbConnection">  the database connection. </param>
        /// <param name="UpdateCommand">    [out] The update command. Just so you can check it! </param>
        /// <param name="errMessage">       Message describing the error if returns null. </param>
        ///
        /// <returns>  A UnidbDataAdpater . </returns>
        ///-------------------------------------------------------------------------------------------------

        public static UniDataAdapter Create_UpdateAll_DataAdapter(DataTable TheTable, string Tablename, UniDbConnection TheDbConnection, List<string>  exclude, out string UpdateCommand, out string errMessage)
        {
            UniDataAdapter TheDbAdapter = null;
            string FUpdateCommand = "";
            errMessage = "";
            try
            {
                string FSelectString = "SELECT * FROM [" + Tablename + "]";
                // OK now create The DataAdapter
                TheDbAdapter = new UniDataAdapter(FSelectString, TheDbConnection);
                TheDbAdapter.SetupEditCommandsForTable(TheTable, Tablename, TheDbConnection);

                // Create the UPdate Command            
                string FUPdateCommandStart = "UPDATE [" + Tablename + "] SET ";

                // build the SET Parameter list
                string SeperatorStr = "";
                string ParmStr = "";
              
                foreach (DataColumn DC in TheTable.Columns)
                {
                    if (!exclude.Exists(delegate(string x) { return x.ToUpper() == DC.ColumnName.ToUpper(); }))
                    {
                        ParmStr += SeperatorStr + DC.ColumnName + " = @" + DC.ColumnName;
                        SeperatorStr = " , ";
                    }
                }
                FUpdateCommand = FUPdateCommandStart + ParmStr;
                // create command
                TheDbAdapter.UpdateCommand.CommandText = FUpdateCommand;
                // no cerate parameters
                /*
                switch (TheDbConnection.SQLServerType)
                {
                    case SQLServer.stAccess:
                        foreach (DataColumn DC in TheTable.Columns)
                        {
                            OleDbParameter OParm = new OleDbParameter("@" + DC.ColumnName, ConvertNetTypeToOledbType(DC.DataType), DC.MaxLength, DC.ColumnName);
                            TheDbAdapter.UpdateCommand.Parameters.Add(OParm);
                        }
                        break;
                    case SQLServer.stMySQL:
                        foreach (DataColumn DC in TheTable.Columns)
                        {
                            MySqlParameter MSParm = new MySqlParameter("@" + DC.ColumnName, ConvertNetTypeToMySQLType(DC.DataType), DC.MaxLength, DC.ColumnName);
                            TheDbAdapter.UpdateCommand.Parameters.Add(MSParm);
                        }
                    ...
                    case SQLServer.stPostgreSQL:
                        foreach (DataColumn DC in TheTable.Columns)
                        {
                            NpgsqlParameter PGParm = new NpgsqlParameter("@" + DC.ColumnName, ConvertNetTypeToPostGreSQLType(DC.DataType), DC.MaxLength, DC.ColumnName);
                            TheDbAdapter.UpdateCommand.Parameters.Add(PGParm);
                        }
                        break;
                    case SQLServer.stText:
                        // YIKES SHOULD NOT BE HERE!
                        throw new Exception("Update Comamnd is Not SUpported for txtdb just yet, sorry!");
                       // break;
                }
*/
                // theorectically this should now work!
            }
            catch (Exception ex)
            {
                ...
                using (StreamWriter SW = new StreamWriter(TxtFilename))
                {
                    ...
                    if (CreateHeader)
                    {
                        ...
                        foreach (DataColumn DC in theDT.Columns)
                        {
                            ...
                            if (colindx < theDT.Columns.Count)
                            {
                                header += fldname + DataSep;
                            }
                            else
                            {
                                header += fldname;
                    ...
                    foreach (DataRow DR in theDT.Rows)
                    {
                        ...
                        {
                            colindx++;
                            string flddata = DR[colindx-1].ToString();
                            if (colindx < theDT.Columns.Count)
                            {
                                dataline += flddata + DataSep;
                            }
                            else
                            {
                                dataline += flddata;
        ...
        public static Type ConvertOledbTypeToNetType(OleDbType DataType)
        {
            Type dbDT = System.Type.GetType("System.Empty");
            if (DataType == OleDbType.Integer) 
            {
                dbDT = System.Type.GetType("System.Int32");
            }
            else
                //        UnsignedInt A 32-bit unsigned integer (DBTYPE_UI4). This maps to UInt32. 
                if (DataType == OleDbType.UnsignedInt)
                {
                    dbDT = System.Type.GetType("System.UInt32");

                }
                else
                    //       SmallInt A 16-bit signed integer (DBTYPE_I2). This maps to Int16. 
                    if (DataType == OleDbType.SmallInt)
                    {
                        dbDT = System.Type.GetType("System.Int16");
                    }
                    else
                        //        UnsignedSmallInt A 16-bit unsigned integer (DBTYPE_UI2). This maps to UInt16. 
                        if (DataType == OleDbType.UnsignedSmallInt)
                        {
                            dbDT = System.Type.GetType("System.UInt16");

                        }
                        else
                            if (DataType == OleDbType.BigInt)
                            {
                                dbDT = System.Type.GetType("System.Int64");
                            }
                            else
                                //        UnsignedBigInt A 64-bit unsigned integer (DBTYPE_UI8). This maps to UInt64. 
                                if (DataType == OleDbType.UnsignedBigInt)
                                {
                                    dbDT = System.Type.GetType("System.UInt64");
                                }
                                else
                                    //        Double A floating point number within the range of -1.79E +308 through 1.79E +308 (DBTYPE_R8). This maps to Double. 
                                    if (DataType == OleDbType.Double)
                                    {
                                        dbDT = System.Type.GetType("System.Double");
                                    }
                                    else
                                        //        Char A character string (DBTYPE_STR). This maps to String. 
                                        //        VarChar A variable-length stream of non-Unicode characters (OleDbParameter only). This maps to String. 
                                        ...
                                        {
                                            // using varchar here becuase char type often limited to 255 chars
                                            dbDT = System.Type.GetType("System.String");
                                        }
                                        else
                                            //  Date Date data, stored as a double (DBTYPE_DATE). The whole portion is the number of days since December 30, 1899, while the fractional portion is a fraction of a day. This maps to DateTime. 
                                            //  DBDate Date data in the format yyyymmdd (DBTYPE_DBDATE). This maps to DateTime. 
                                            ...
                                            //  DBTimeStamp Data and time data in the format yyyymmddhhmmss (DBTYPE_DBTIMESTAMP). This maps to DateTime. 
                                            //        Filetime A 64-bit unsigned integer representing the number of 100-nanosecond intervals since January 1, 1601 (DBTYPE_FILETIME). This maps to DateTime. 
                                            if (DataType == OleDbType.DBTimeStamp)
                                            {
                                                // using DBTimeSTamp to conve full info
                                                dbDT = System.Type.GetType("System.DateTime");
                                            }
                                            else
                                                // Boolean A Boolean value (DBTYPE_BOOL). This maps to Boolean. 
                                                if (DataType == OleDbType.Boolean)
                                                {
                                                    dbDT = System.Type.GetType("System.Boolean");
                                                }
                                                else
                                                    //  Decimal A fixed precision and scale numeric value between -1038 -1 and 10 38 -1 (DBTYPE_DECIMAL). This maps to Decimal. 
                                                    //  Currency A currency value ranging from -263 (or -922,337,203,685,477.5808) to 2 63 -1 (or +922,337,203,685,477.5807) with an accuracy to a ten-thousandth of a currency unit (DBTYPE_CY). This maps to Decimal. 
                                                    //  Numeric An exact numeric value with a fixed precision and scale (DBTYPE_NUMERIC). This maps to Decimal. 
                                                    if (DataType == OleDbType.Currency)
                                                    {
                                                        dbDT = System.Type.GetType("System.Decimal");
                                                    }
                                                    else
                                                        // Single A floating point number within the range of -3.40E +38 through 3.40E +38 (DBTYPE_R4). This maps to Single. 
                                                        if (DataType == OleDbType.Single)
                                                        {
                                                            dbDT = System.Type.GetType("System.Single");
                                                        }
                                                        else
                                                            //        UnsignedTinyInt A 8-bit unsigned integer (DBTYPE_UI1). This maps to Byte. 
                                                            if (DataType == OleDbType.UnsignedTinyInt)
                                                            {
                                                                dbDT = System.Type.GetType("System.Byte");
                                                            }
                                                            else
                                                                //  TinyInt A 8-bit signed integer (DBTYPE_I1). This maps to SByte. 
                                                                if (DataType == OleDbType.TinyInt)
                                                                {
                                                                    dbDT = System.Type.GetType("System.SByte");
                                                                }
        ...
        ///-------------------------------------------------------------------------------------------------
        /// <summary>   Creates a double value that provides a unique Seed for ID.  Based on Clock Ticks
        ///             since 1/1/2013. </summary>
        ///
        /// <returns>   Unique Double Value. </returns>
        ///-------------------------------------------------------------------------------------------------

        public static Int64 CreatePrimaryIDIntSeed()
        {
            Int64 TheKey = 0;
            ...
            DateTime NowTime = new DateTime();
            NowTime = DateTime.Now;
            DateTime keyDT = new DateTime(2016, 1, 1);
            long TheBigTicks = NowTime.Ticks - keyDT.Ticks;
            TheKey = TheBigTicks;
            //TheKey = Convert.ToDouble(TheBigTicks / SpanTime);
            return TheKey;
        ...
        /// <summary> The sqlfloat. </summary>
        /// <remarks> floating point System.Single 32 bit</remarks>
        public const int siSQLFLOAT = 4; // System.Single 32 bit
        /// <summary> The sqldouble. </summary>
        /// <remarks>Floating point System.Double 64 bit</remarks>
        ...
        static string[] SQL_MYSQL = new string[SQLTYPENUMBER] { "MYSQL", "VARCHAR", "MEDIUMTEXT", "DECIMAL", "FLOAT", "DOUBLE", "SMALLINT", "INT", "BIGINT", "DATE", "TIME", "TINYINT", "BOOLEAN", "DATETIME" };
        ...
        ///-------------------------------------------------------------------------------------------------
        /// <summary>   Puts a data in text line. </summary>
        ///
        ...
        static public string PutDataInTextLine(string[] Data, DataFormat FormatType, int ColWidth)
        {
            string OutStr = "";
            for (int index = 0; index < Data.Length; index++)
            {
                string DataStr = Data[index];
                if (DataStr != "")
                {
                    switch (FormatType)
                    {
                        case UniDB.Tools.DataFormat.CommaDelimited:
                            if (index > 0) DataStr = "," + DataStr;
                            break;
                        case UniDB.Tools.DataFormat.TabDelimited:
                            if (index > 0) DataStr = "\t" + DataStr;
                            break;
                        case UniDB.Tools.DataFormat.SpaceDelimited:
                            if (index > 0) DataStr = " " + DataStr;
                            break;
                        case UniDB.Tools.DataFormat.FixedFields:
                            DataStr = DataStr.PadLeft(ColWidth);
                            break;
                    }
                    OutStr += DataStr;
                }
        ...
        static public string PutDataInTextLine(int[] Data, DataFormat FormatType, int ColWidth)
        {
            string OutStr = "";
            for (int index = 0; index < Data.Length; index++)
            {
                string DataStr = Data[index].ToString();
                switch (FormatType)
                {
                    case UniDB.Tools.DataFormat.CommaDelimited:
                        if (index > 0) DataStr = "," + DataStr;
        ...
        static public string PutDataInTextLine(double[] Data, DataFormat FormatType, int ColWidth, string FormatString)
        {
            string OutStr = "";
            for (int index = 0; index < Data.Length; index++)
            {
                string DataStr = Data[index].ToString(FormatString);
                switch (FormatType)
                {
                    case UniDB.Tools.DataFormat.CommaDelimited:
                        if (index > 0) DataStr = "," + DataStr;
        ...
        ///                   If valueStr is not in a Integer or Float format, or the format exceeds the MaxSize or MinSize of Int32, then IsError true and result 0, 
        ///                   with a message of what happened in errMessage.  If in a float format, this is first converted to a Double, then to an Int32,
        ...
        /// <remarks>Uses the Net Convert class methods, but traps exceptions from Convert and indicates errors with iserror and errMessage.
        ///                   If valueStr is not in a Integer or Float format, or the format exceeds the MaxSize or MinSize of Int16, then IsError true and result 0, 
        ///                   with a message of what happened in errMessage.  If in a float format, this is first converted to a Double, then to an Int16,
        ///                   which rounds the decimal value.
        ///                   </remarks>
        /// <param name="valueStr">   The value string. </param>
        /// <param name="IsError">    The is error. </param>
        ...
        /// <remarks>Uses the Net Convert class methods, but traps exceptions from Convert and indicates errors with iserror and errMessage.
        ///                   If valueStr is not in a Integer or Float format, or the format exceeds the MaxSize or MinSize of Int64, then IsError true and result 0, 
        ///                   with a message of what happened in errMessage.  If in a float format, this is first converted to a Double, then to an Int64,
        ///                   which rounds the decimal value.
        ///                   </remarks>
        /// <param name="valueStr">   The value string. </param>
        /// <param name="IsError">    The is error. </param>
        ...
        /// <remarks>Uses the Net Convert class methods, but traps exceptions from Convert and indicates errors with iserror and errMessage.
        ///                   If valueStr is not in a Float format, or the format exceeds the MaxSize or MinSize of Double then IsError true and result 0, 
...
//            List<ColumnInfo> FColInfoList = new List<ColumnInfo>();
        ...
        static public DbCommand CreateUpdateCommand(List<ColumnInfo> ColInfoList, string WHereClause ,string Tablename, UniDbConnection DbCon)
        {
            ...
            foreach (UniDB.Tools.ColumnInfo CI in ColInfoList)
            {
                if (CI.ColumName != "ID")
                {
                    ColumnSetList += Tools.BracketIt(DbCon.SQLServerType, CI.ColumName) + " =  @" + CI.ColumName + " , ";
                }
            }
            ColumnSetList = ColumnSetList.Substring(0, ColumnSetList.Length - 3) + " ";
            ...
            if (WHereClause != "")
            {
                CommandInsertString += " WHERE " + WHereClause;
            } 
...
//                  TempCom.Parameters.Add(DbCon.CreateDbParameter(CI.ColumName, CI.DataType, CI.Width));
        ...
        static public DbCommand CreateUpdateCommandWhere(DataTable DT, string Tablename, string WhereClause, UniDbConnection DbCon)
        {
            List<ColumnInfo> ColInfoList = new List<ColumnInfo>();
            foreach (DataColumn DC in DT.Columns)
            {
            ...
            return CreateUpdateCommand(ColInfoList, WhereClause, Tablename, DbCon);
        }
        static public DbCommand CreateUpdateCommand(DataTable DT, string Tablename, UniDbConnection DbCon)
        {
            ...
            //}
            // 
            return CreateUpdateCommand(ColInfoList,"ID = @ID",Tablename,DbCon);
        }
    }
